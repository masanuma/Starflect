...
`;
      ...
      const randomId = Math.random().toString(36).substring(2, 8);
        ...
        【金銭運】
          ...
          - 長い説明や追加コメントは一切禁止
          - 指定された形式以外の追加テキストは禁止
          - 【マークダウン禁止】：マークダウン記号（**、-、###など）は絶対に使用しないでください。強調は「」や【】を使用してください。
...
【金銭運】
      ...
      const { getOpenAIApiKey, getGeminiApiKey, isApiKeyAvailable, isGeminiAvailable, debugEnvConfig } = await import('../config/env');
      ...
      // 常にプロキシ（/api）経由で呼び出す（CSPエラーとService Workerエラー回避のため）
      const endpoint = "/api/gemini-proxy";
      const model = "gemini-pro"; // 確実に動作するモデル
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "あなたは宇宙の詩人であり、魂の通訳者です。初心者向けの12星座占いを提供しますが、その言葉は美しく、心に深く刻まれるものでなければなりません。星々の囁きを、優しく印象的な物語として紡いでください。"
            },
            {
              role: "user",
      ...
      if (!response.ok) {
        throw new Error(`${isGeminiAvailable() ? 'Gemini' : 'OpenAI'} API呼び出しに失敗しました`);
      }

      const data = await response.json();
      const aiResult = cleanAIOutput(data.choices[0]?.message?.content || '');
...
`;
      ...
      debugLog('🔍 【3天体占いAI呼び出し】Gemini対応プロキシ使用');
      // Level1同様にAPIを直接呼び出し
      const { isGeminiAvailable } = await import('../config/env');
      // 常にプロキシ（/api）経由で呼び出す（CSPエラー回避のため）
      const endpoint = "/api/gemini-proxy";
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: "あなたは魂の深層を読み解く熟練の占星術師です。太陽・月・上昇星座の響き合いから、その人の本質を射抜くような印象的な占いを提供してください。"
            },
            {
              role: 'user',
              content: analysisPrompt
      ...
      if (!response.ok) {
        throw new Error(`${isGeminiAvailable() ? 'Gemini' : 'OpenAI'} API error: ${response.status}`);
      }
...
`;
      ...
      const randomId = Math.random().toString(36).substring(2, 8);
      let analysisPrompt = `
        あなたは経験豊富な西洋占星術師であり、星々の声を物語として紡ぐ語り部でもあります。
        以下の3要素を統合し、クライアントの人生に深く寄り添う詳細な占いを行ってください。
        ...
        - 占い実行時刻: ${new Date().toLocaleString('ja-JP')}
        ${previousLevel3Context}
        ${timeContext}
        
        【重要：占いの指針】：
        1. 【技術的根拠の明示】: 「出生時の○○座にある太陽と、現在の△△座を運行中の木星が〜」といった、どの星とどの星がどのように響き合っているからその運勢になるのか、という具体的な占星術的根拠を必ず文章に含めてください。
        2. 【具体的かつ多角的】: 「良いこと」だけでなく「注意点や葛藤」も具体的に伝えてください。
        3. 【期間に応じた詳しさ】: 1セクション 150-200文字程度で、物語を読むような深みのある内容にしてください。
        4. 【ふわっとした表現の排除】: 「運気が良いです」だけでなく、「具体的にどのようなチャンスが訪れるのか」「何を心がければ良いのか」を血の通った言葉で伝えてください。
        ...
        ${(() => {
          ...
          if (period === 'today' || period === 'tomorrow') {
            return `
            - 出生時の天体と今日の天体の角度（アスペクト）を重視した、当日の運勢変化
            - 朝・昼・夕方・夜など、天体の動きに合わせた時間帯別のアドバイス
            - 今すぐ実践できる具体的な開運アクション
            - 今日/明日という限られた時間の中で最大限に活かせるギフトについて`;
          } else if (period === 'thisWeek' || period === 'nextWeek') {
            return `
            - 週の始まりから終わりまでの天体の軌道が、出生図にどう影響するか
            - 前半の緊張と後半の調和など、1週間の中でのドラマチックな展開
            - 具体的なラッキーデーとその理由（どの星が重なるかなど）
            - 週単位での目標設定に向けた、星々からの戦略的アドバイス`;
          } else if (period === 'thisMonth' || period === 'nextMonth') {
            return `
            - 1ヶ月を通じた主要な天体の逆行や星座移動が出生図に与える影響
            - 新月・満月のサイクルと、あなたの出生時の月とのシンクロニシティ
            - 月間を通じた「魂の成長テーマ」と、具体的課題
            - 月内の重要な転換点となる日付とその占星術的理由`;
          } else if (period === 'threeMonths') {
            return `
            - 3ヶ月という季節の移ろいの中で、土星や木星があなたのどの天体に働きかけるか
            - 段階的な変化のプロセス。今、何を手放し、何を新しく受け入れるべきか
            - 中期的なプロジェクトや人間関係の発展における、具体的なマイルストーン
            - この3ヶ月で達成すべき「星からの課題」について`;
          } else if (period === 'sixMonths' || period === 'oneYear') {
            return `
            - 外惑星（天王星・海王星・冥王星）があなたの人生のどの領域を根底から変えようとしているか
            - 半年〜1年という長い時間軸の中で完成される、大きな運命のタペストリー
            - 人生の大きな転換期としての意味。過去の清算と未来への種まき
            - 長期ビジョンを実現するために、どの天体のエネルギーを味方につけるべきか`;
          } else {
            return `
            - 人生の大きな節目、世代的な変化の中でのあなたの役割
            - 長期的な人生計画・キャリア設計に対する、星々からの壮大な示唆
            - 魂の使命に目覚めるための、長期的なプロセスと試練
            - あなたという存在が世界に対して放つ、固有の光の磨き方`;
          }
        })()}
        ...
        - 専門用語を使いつつも、その意味が直感的に伝わるような美しい日本語を使うこと
        
        【絶対に守るべき重要ルール】：
        - マークダウン記号（**、###、-など）は一切使用禁止
        - 季節や時期に関する表現（夏のエネルギー、今の時期、季節が〜など）は一切使用禁止
        - 文章はですます調で親しみやすく記載
        - 【重要】強調したい言葉がある場合は、マークダウン（**）を使わず、代わりに「」や【】を使用してください。
        ...
        【厳格な出力指示（違反は絶対禁止）】：
        以下の5つのセクションのみ出力してください。他のセクションは一切出力禁止。
        各セクションは150-200文字程度で、たっぷりと記述してください。
        強調のために「**」は絶対に使用しないでください。
...
`;
      ...
      debugLog('🔍 【Level3占い生成】Gemini対応プロキシ使用');
      debugLog('🔍 【Level3占い生成】includeImportantDays:', includeImportantDays);
      debugLog('🔍 【Level3占い生成】selectedPeriod:', selectedPeriod);
      
      const { isGeminiAvailable } = await import('../config/env');
      const endpoint = isGeminiAvailable() ? "/api/gemini-proxy" : "/api/openai-proxy";
      const model = isGeminiAvailable() ? "gemini-1.5-flash" : "gpt-4o-mini";

      // Level1・Level2同様にAPIを直接呼び出し
      const response = await fetch(endpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: "あなたは時空を超えた知恵を持つ、究極の星読み師です。クライアントの運命のうねりを捉え、魂を鼓舞するような力強く印象的な言葉で、未来への指針を伝えてください。"
            },
            {
              role: 'user',
              content: analysisPrompt
  ...
  const handleGenerateLevel3Analysis = useCallback(async () => {
    ...
    if (cachedAnalysis) {
      try {
        ...
        if (now < expiryTime) {
          ...
          // 新しい形式（tenPlanetSummaryがある）か、エラー状態でないかチェック
          if (analysis && analysis.tenPlanetSummary) {
            debugLog('🔍 【キャッシュ有効】新形式のAI分析を読み込みます');
            setLevel3Analysis(analysis);
            setIsGeneratingLevel3Analysis(false); // ローディング状態を解除
            return;
          } else {
            debugLog('🔍 【キャッシュ形式不適合】古い形式のため再生成が必要です');
    ...
    try {
      ...
      if (!analysis || (analysis.isError && !analysis.tenPlanetSummary)) {
        throw new Error('AI分析の結果が空かエラー状態です');
      }
  ...
  useEffect(() => {
    ...
    if (level3Analysis && !level3Analysis.tenPlanetSummary && !level3Analysis.isError) {
    ...
    if (currentLevel >= 2 && horoscopeData && birthData && !level3Analysis && !isGeneratingLevel3Analysis) {
